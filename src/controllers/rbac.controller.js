...
class RBAC {
  ...
  async createOnePermission(request, response) {
    try {
      ...
      if (!description) {
        return response
          .status(400)
          .send({ message: "A descrição é um campo obrigatório" });
      }
      ...
      return response
        .status(400)
        .send({ message: "A permissão não pôde ser criada!" });
    }
  }
  ...
  async createOneRole(request, response) {
    try {
      ...
      return response
        .status(201)
        .send({ message: "A Função foi criada!", data });
    } catch (error) {
      console.log(error.message);
      return response
        .status(400)
        .send({ message: "A Função não pôde ser criada!" });
    }
  }
  ...
  async addPermissionToRole(request, response) {
    try {
      ...
      if (!permission) {
        return response
          .status(400)
          .send({ message: "A permissão não existe." });
      }
  ...
  async addRoleToUser(request, response) {
    try {
      ...
      if (!userId) {
        return response
          .status(400)
          .send({ message: "O id do usuário é um campo obrigatório" });
      }
